...
//        获取mao中所有一级菜单的集合
    ...
    @Override
    public Result currentTopMenuOne() {
        Subject subject = SecurityUtils.getSubject();
        ActiveUser activeUser =(ActiveUser) subject.getPrincipal();
        Integer userId = activeUser.getSysUser().getId();
//        查询出用户所有的菜单
        List<SysPermissionVo> sysPermissionVos = sysPermissionMapper.selectList(userId, Constant.PERMISSION_TYPE_MENU);
//      将数组转化为前端所能接受的json格式，转换为父子关系
        Map<Integer,SysPermissionVo> menuOne=new HashMap<>();
        for (SysPermissionVo vo : sysPermissionVos) {
//            获取父菜单如果为0则为一级
            Integer parentId = vo.getParentId();
            if (parentId.equals(Constant.MENUONE_LV1)){
//                初始化一级菜单容器
                vo.setChildren(new ArrayList<SysPermissionVo>());
                menuOne.put(vo.getId(),vo);
        ...
        for (SysPermissionVo vo : sysPermissionVos) {
//            判断一级菜单容器中是否有当前菜单的父菜单
            if(menuOne.containsKey(vo.getParentId())){
//                获取`当前菜单的父菜单
                SysPermissionVo vo1 = menuOne.get(vo.getParentId());
//                将当前菜单放入到一级菜单下
                vo1.getChildren().add(vo);
...
//        获取mao中所有一级菜单的集合
        Collection<SysPermissionVo> values = menuOne.values();
    ...
    @Override
    public Result currentTopMenuTwo() {
        Subject subject = SecurityUtils.getSubject();
        ActiveUser activeUser =(ActiveUser) subject.getPrincipal();
        Integer userId = activeUser.getSysUser().getId();
//        查询出用户所有的菜单
...
//            获取父菜单如果为0则为一级
            Integer parentId = vo.getParentId();
            if (parentId.equals(Constant.MENUTWO_LV1)){
//                初始化一级菜单容器
                vo.setChildren(new ArrayList<SysPermissionVo>());
                menuOne.put(vo.getId(),vo);
